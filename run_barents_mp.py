...
def get_particle_set(fieldset, run3D=False):
    ...
    npart = 500  # number of particles to be released
    lon = np.linspace(11.0 , 6.5, npart, dtype=np.float32)
    lat = np.linspace(65.8 , 66.4, npart, dtype=np.float32)
    repeatdt = delta(days=10)  # release from the same set of locations every 3 hours

   
    return ParticleSet.from_list(fieldset, PlasticParticle, lon=lon, lat=lat, repeatdt=repeatdt)

                                     
def run_barents_norw_mp(outfile, run3D=False):
    ...
    for d in range(int(ndays/2)):
        ...
        pfile.write(pset, pset[0].time)
        if day > 365: #no particle release after 1 year
            pset.repeatdt = None
